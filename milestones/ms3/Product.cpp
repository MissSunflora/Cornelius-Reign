...
//	//The Destructor
	...
	bool Product::operator>(const Product& object) const
	{
		...
		return (strcmp(m_name, object.m_name) > 0) ? true : false;
	...
	std::istream& Product::read(std::istream& in, bool interractive)
	{
		if (interractive == false)
		{
			...
			char* tax = new char[max_length_sku + 1];
			//int  qtyA;
			//int qtyN;
			...
			strcpy(m_unit, unit);
			//cout << unit;
			in.getline(price, ama::max_length_sku, ',');
			//cout << price;
			m_price = stod(price);
			in.getline(tax, ama::max_length_sku, ',');
			//cout << tax;
			//if (tax == "0")
			//	taxStatus == false;
			//else
			//	taxStatus == true;
			in.getline(qtyA, ama::max_length_sku, ',');
			//cout << qtyA;
			m_qtyOnhand = stod(qtyA);
			in.getline(qtyN, ama::max_length_sku, '\n');
			//cout << qtyN;
